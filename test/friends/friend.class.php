	...
	function selectFriend($_friend_id) {
		...
		if($_friend_id != "") {
			...
			$sql = "SELECT /* USE MASTER */ f.friend_id, f.bugzilla_id, f.first_name, f.last_name, f.date_joined, f.is_anonymous, f.is_benefit,
					...
					FROM chris_friends as f 
					LEFT JOIN (SELECT friend_id, MAX(date_expired) AS date_expired FROM chris_friends_contributions GROUP BY friend_id) fc_temp 
